...
/* ------------------------------------------------------------------ */
/*  Chart data builders (derived from real API data — no hardcoded samples) */
/* ------------------------------------------------------------------ */

/** Convert the 30-day trend into the shape the line chart expects. */
function buildTrendLineData(trend: TrendPoint[]) {
  return (trend ?? []).map((t) => ({
    ...
    Submitted: t.total,
    Resolved: t.resolved,
    Escalated: Math.max(0, t.total - t.resolved),
  }));
}

/** Convert department performance into avg-resolution-days bars (descending). */
function buildResolutionTimeData(deptPerf: DeptPerf[]) {
  return (deptPerf ?? [])
    .filter((d) => d.avgResolutionHours !== null)
    .map((d) => ({
      department: d.departmentName,
      days: Math.round(((d.avgResolutionHours ?? 0) / 24) * 10) / 10,
    }))
...
/** Build the priority pie data from a real priority breakdown. */
function buildPriorityData(
  breakdown: { key: string; count: number }[] | undefined,
) {
  if (!breakdown || breakdown.length === 0) {
    return [
      { name: "P1 – Critical", value: 0, color: PRIORITY_COLORS.P1 },
      { name: "P2 – High", value: 0, color: PRIORITY_COLORS.P2 },
      { name: "P3 – Medium", value: 0, color: PRIORITY_COLORS.P3 },
      { name: "P4 – Low", value: 0, color: PRIORITY_COLORS.P4 },
    ];
  }
  return breakdown.map((b) => ({
    name: PRIORITY_LABELS[b.key] ?? b.key,
    value: b.count,
    color: PRIORITY_COLORS[b.key] ?? "#6b7280",
  }));
}
...
function avgResolutionHours(deptPerf: DeptPerf[]): number | null {
  ...
  return Math.round(
    valid.reduce((s, d) => s + (d.avgResolutionHours ?? 0), 0) / valid.length,
  );
}
...
export default function ReportsPage() {
  ...
  /** Convert the selected range preset into { from, to, days } for the API. */
  function resolveRange(preset: string): {
    ...
    switch (preset) {
      case "Last 7 days": {
        ...
        return { from: toIso(d), to: today, days: 7 };
      }
      case "Last 90 days": {
        const d = new Date();
        d.setDate(d.getDate() - 90);
        return { from: toIso(d), to: today, days: 90 };
      }
      case "This Year": {
        const d = new Date(now.getFullYear(), 0, 1);
        return { from: toIso(d), to: today, days: 365 };
      }
      case "All Time":
      ...
      default: {
        ...
        return { from: toIso(d), to: today, days: 30 };
      }
    }
  }
  ...
  const fetchAll = useCallback(async () => {
    setLoading(true);
    try {
      // Manual date inputs take precedence over the preset when set.
  ...
  }, [from, to, dateRangeFilter]);

  // Changing the preset refetches with the new range.
  useEffect(() => {
    if (!from && !to) fetchAll();
  }, [dateRangeFilter, fetchAll, from, to]);
  ...
  // Derive the priority pie from real data (was a hardcoded array).
  const priorityData = buildPriorityData(priorityBreakdown);
  // Derive trend + resolution-time charts from real fetched data.
  const trendLineData = buildTrendLineData(trend);
  const resolutionTimeData = buildResolutionTimeData(deptPerf);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            {formatDateLong()} &middot; Performance metrics and analytics
            overview
          </p>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <div className="relative" ref={exportDropdownRef}>
            <button
              className="inline-flex items-center gap-1.5 rounded-lg border border-border bg-card px-3 py-1.5 text-xs font-semibold text-foreground shadow-sm transition-colors hover:bg-neutral-50"
            ...
            {showExportDropdown && (
              <div className="absolute right-0 top-full z-20 mt-1 w-36 rounded-lg border border-border bg-card py-1 shadow-lg">
                {["PDF", "CSV", "Excel"].map((format) => (
                  <button
                    key={format}
                    className="flex w-full items-center gap-2 px-3 py-2 text-sm text-foreground transition-colors hover:bg-green-50 hover:text-green-700"
      ...
      <div className="flex flex-wrap items-center gap-2">
          ...
          <button
            ...
            className={`rounded-full px-3 py-1.5 text-xs font-medium transition-colors ${
              dateRangeFilter === option
                ...
                : "bg-card text-muted-foreground border border-border hover:border-green-300 hover:text-green-700"
            }`}
        ...
        <div className="flex items-center gap-2 rounded-lg border border-border bg-card px-3 py-1.5 shadow-sm">
          ...
          <input
            ...
            className="rounded-md border border-border px-2 py-1 text-xs outline-none focus:border-green-600 focus:ring-1 focus:ring-green-600/20"
          />
          ...
          {(from || to) && (
            <button
              className="rounded-lg px-2 py-1 text-xs font-medium text-foreground transition-colors hover:bg-neutral-50"
              onClick={() => {
                setFrom("");
                setTo("");
              }}
      ...
      {/* ─────────── Success toast ─────────── */}
      {reminderSuccess && (
        <div className="flex items-center gap-2 rounded-lg border border-green-200 bg-green-50 px-4 py-3 text-sm text-green-700 shadow-sm">
          <svg
      ...
      {stats && (
        <div className="grid grid-cols-2 gap-4 lg:grid-cols-4">
          ...
          <div className="rounded-xl border border-border bg-card p-4 shadow-sm">
            ...
            <div className="mt-3">
              <p className="text-2xl font-bold text-foreground">
                {avgResolutionHours(deptPerf) !== null
                  ? `${avgResolutionHours(deptPerf)}h`
                  : "N/A"}
              </p>
              <p className="text-xs font-medium text-muted-foreground">
                Avg Response Time
          ...
          <div className="rounded-xl border border-border bg-card p-4 shadow-sm">
            ...
            <div className="mt-3">
              <p className="text-2xl font-bold text-foreground">
                {Math.max(0, 100 - overallBreachRate(stats) * 2)}%
              </p>
              <p className="text-xs font-medium text-muted-foreground">
                Compliance Score
              </p>
          ...
          <div className="p-6">
            <div className="mb-2">
              ...
              <p className="text-xs text-muted-foreground">
                Breakdown of all complaints by current status
              </p>
            </div>
            <div className="h-64">
        ...
        <div className="rounded-xl border border-border bg-card shadow-sm">
          <div className="p-6">
            <div className="mb-2">
              ...
              <p className="text-xs text-muted-foreground">
                Last 30 days — new vs resolved complaints
              </p>
            </div>
            <div className="h-64">
              <ResponsiveContainer width="100%" height="100%">
        ...
        <div className="rounded-xl border border-border bg-card shadow-sm">
          <div className="p-6">
            <div className="mb-2">
              ...
              <p className="text-xs text-muted-foreground">
                12-month overview — submitted, resolved & escalated
              </p>
            </div>
            <div className="h-72">
              <ResponsiveContainer width="100%" height="100%">
        ...
        <div className="rounded-xl border border-border bg-card shadow-sm">
          <div className="p-6">
            <div className="mb-2">
              ...
              <p className="text-xs text-muted-foreground">
                Average days to resolve — color-coded by performance
              </p>
            </div>
            <div className="h-72">
              <ResponsiveContainer width="100%" height="100%">
            ...
            <div className="mt-3 flex flex-wrap justify-center gap-x-5 gap-y-1">
              ...
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-xs text-muted-foreground">5–10 days</span>
              </div>
              <div className="flex items-center gap-1.5">
      ...
      <div className="rounded-xl border border-border bg-card shadow-sm">
        <div className="p-6">
          <div className="mb-2">
            ...
            <p className="text-xs text-muted-foreground">
              Complaints categorized by priority level (P1–P4)
            </p>
          </div>
            ...
            <div className="flex flex-col justify-center gap-4 md:mt-8">
              <div className="text-center md:text-left">
                <p className="text-3xl font-bold text-foreground">
                  {priorityData.reduce((s, d) => s + d.value, 0)}
                </p>
                <p className="text-xs font-medium text-muted-foreground">
                  Total Complaints
                </p>
                ...
                {priorityData.map((d) => (
                  <div key={d.name} className="flex items-center gap-3">
                    ...
                    <span className="text-xs text-muted-foreground">
                      ...
                      {Math.round(
                        (d.value /
                          priorityData.reduce((s, p) => s + p.value, 0)) *
                          100,
                      )}
                      %)
                    </span>
          ...
          {reminders.length === 0 ? (
            <div className="flex flex-col items-center py-8 text-muted-foreground">
              ...
              <p className="text-xs mt-1">
                Click &ldquo;New Reminder&rdquo; to schedule a follow-up
              </p>
            </div>
          ) : (
            <div className="max-h-64 overflow-y-auto space-y-3 pr-1">
              {reminders.map((r) => (
                <div
                  key={r.id}
                  className="flex items-start gap-3 rounded-lg border border-border bg-card p-3 shadow-sm transition-colors hover:border-green-200"
                >
                  {/* Bell icon */}
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="text-sm font-semibold text-foreground truncate">
                      {r.ticket?.ticketCode ?? "Ticket"} &mdash;{" "}
                      {r.ticket?.subject ?? "N/A"}
                    </p>
                    {r.note && (
                  ...
                  {/* Delete button */}
                  <button
                    className="rounded-lg px-3 py-1.5 text-xs font-medium text-muted-foreground transition-colors hover:text-red-500 hover:bg-neutral-50 disabled:opacity-60 disabled:cursor-not-allowed"
      ...
      <div className="rounded-xl border border-border bg-card shadow-sm">
        <div className="p-6">
          <div className="mb-4">
            ...
            <p className="text-xs text-muted-foreground">
              Comparison across departments for the selected period
            </p>
          </div>
          {deptPerf.length === 0 ? (
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-green-50 text-left">
                    <th className="px-5 py-3.5 text-xs font-semibold uppercase tracking-wider text-green-800">
                      Department
                    </th>
                    <th className="px-5 py-3.5 text-xs font-semibold uppercase tracking-wider text-green-800 text-right">
                      Total
                    </th>
                    <th className="px-5 py-3.5 text-xs font-semibold uppercase tracking-wider text-green-800 text-right">
                      Resolved
                ...
                <tbody>
                  {deptPerf.map((d, i) => (
                    <tr
                      key={d.departmentId}
                      className={`${i % 2 === 0 ? "bg-card" : "bg-neutral-50/50"} border-b border-border transition-colors hover:bg-green-50/30`}
                    >
                      <td className="px-5 py-3.5">
                        <div className="flex items-center gap-2">
                          <div className="flex h-7 w-7 shrink-0 items-center justify-center rounded-md bg-green-100 text-green-700 text-xs font-bold">
                        ...
                        {d.avgResolutionHours !== null ? (
                          <span
                            className={`font-medium ${
                              d.avgResolutionHours <= 48
                                ? "text-green-600"
                                : d.avgResolutionHours <= 96
                                  ? "text-amber-600"
                                  : "text-red-600"
                            }`}
                          >
                        ...
                        <div className="flex items-center gap-2.5">
                          <div className="h-2 w-24 rounded-full bg-neutral-200 overflow-hidden">
                            <div
                              ...
                              style={{
                                width: `${Math.min(d.breachRate, 100)}%`,
                              }}
                            />
                          </div>
                          <span
                        ...
                        <div className="flex items-center gap-2.5">
                          <div className="h-2 w-24 rounded-full bg-neutral-200 overflow-hidden">
                            <div
                              ...
                              style={{
                                width: `${Math.min(d.reopenRate, 100)}%`,
                              }}
                            />
                          </div>
                          <span
      ...
      {showReminderModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          />
          <div className="relative z-10 w-full max-w-md rounded-2xl bg-card p-0 shadow-2xl">
            <div className="p-6">
              <div className="mb-4 flex items-center justify-between">
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <select
                    ...
                    className="w-full rounded-lg border border-border bg-card px-3 py-2.5 text-sm outline-none focus:border-green-600 focus:ring-1 focus:ring-green-600"
                  >
                ...
                <div>
                  ...
                  <textarea
                    ...
                    className="w-full rounded-lg border border-border px-3 py-2 text-sm outline-none focus:border-green-600 focus:ring-1 focus:ring-green-600/20 resize-none"
                  />
                </div>
                ...
                <div>
                  ...
                  <input
                    ...
                    className="w-full rounded-lg border border-border px-3 py-2 text-sm outline-none focus:border-green-600 focus:ring-1 focus:ring-green-600/20"
                  />
                </div>
              </div>
              <div className="mt-4 flex justify-end gap-2">
                ...
                <button
                  ...
                  disabled={
                    !selectedTicketId || !remindAt || submittingReminder
                  }
                >
