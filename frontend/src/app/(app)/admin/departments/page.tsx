...
export default function DepartmentsAdminPage() {
  ...
  async function handleDelete(d: Department) {
    if (!confirm(`Delete department "${d.name}"? This is blocked if it has active users or open tickets.`)) return;
    try {
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-semibold text-neutral-800">Departments</h1>
          <p className="text-sm text-neutral-500">{departments.length} department(s)</p>
        </div>
        <button className="rounded-lg bg-teal-600 px-4 py-2.5 text-sm font-semibold text-white transition-colors hover:bg-teal-700" onClick={openCreate}>+ Add Department</button>
      </div>
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {departments.length === 0 && !loading ? (
                  <tr><td colSpan={4} className="px-4 py-10 text-center text-neutral-400">No departments.</td></tr>
                ) : (
                  departments.map((d) => (
                    <tr key={d.id} className="border-b border-neutral-100 hover:bg-neutral-50">
                      ...
                      <td className="px-4 py-3 font-mono text-xs text-teal-700">{d.code}</td>
                      <td className="max-w-[300px] truncate px-4 py-3 text-neutral-600">
                        {d.description ?? "—"}
                      </td>
                      <td className="px-4 py-3">
                        <div className="flex gap-2">
                          <button className="rounded-lg px-3 py-1.5 text-xs font-medium text-neutral-700 transition-colors hover:bg-neutral-100" onClick={() => openEdit(d)}>Edit</button>
                          <button className="rounded-lg px-3 py-1.5 text-xs font-medium text-neutral-700 transition-colors hover:bg-neutral-100" onClick={() => handleDelete(d)}>Delete</button>
                        </div>
                      </td>
                    </tr>
                  ))
          ...
          <div className="relative z-10 w-full max-w-md rounded-2xl bg-white p-0 shadow-2xl">
            <div className="p-6">
              <div className="mb-4 flex items-center justify-between">
                <h2 className="text-lg font-semibold text-neutral-900">{editing ? "Edit Department" : "Add Department"}</h2>
              ...
              <div className="space-y-4">
                ...
                <div>
                  <label className="mb-1 block text-sm font-medium text-neutral-700">Code * <span className="text-neutral-400">(unique, e.g. WORKS)</span></label>
                  <input value={code} onChange={(e) => setCode(e.target.value.toUpperCase())} className={inputClass} maxLength={20} />
                </div>
                <div>
