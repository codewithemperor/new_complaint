...
export default function SlaDashboardPage() {
  ...
  const fetchTickets = useCallback(async () => {
    ...
    setError(null);
    try {
      const data = await api.get<{ items: Ticket[] }>(`/sla/breaching?view=${view}`);
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-semibold text-neutral-800">Response Deadlines</h1>
        ...
        {(["all", "breached", "warning"] as View[]).map((v) => (
          <button
            ...
            className={`rounded-md px-4 py-2 text-sm font-medium capitalize transition-colors ${
              view === v ? "bg-white text-teal-700 shadow-sm" : "text-neutral-500 hover:text-neutral-700"
            }`}
          >
            {v}
          </button>
      ...
      {tickets.length === 0 && !loading ? (
        <div className="rounded-xl border border-neutral-200 bg-white shadow-sm">
          <div className="p-6">
            <p className="py-8 text-center text-neutral-500">No tickets in this view.</p>
          </div>
        </div>
      ) : (
        <div className="rounded-xl border border-neutral-200 bg-white shadow-sm">
                ...
                <tbody>
                  {tickets.map((t: any) => (
                    <tr key={t.id} className="border-b border-neutral-100 hover:bg-neutral-50">
                      <td className="px-4 py-3 font-mono text-xs text-teal-700">{t.ticketCode}</td>
                      <td className="max-w-[200px] truncate px-4 py-3 font-medium text-neutral-800">{t.subject}</td>
                      <td className="px-4 py-3 text-neutral-600">{t.department?.name ?? "—"}</td>
