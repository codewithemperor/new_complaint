...
const PRIORITY_OPTIONS = [
  ...
  { value: "P4", label: "P4 — Low", color: "bg-emerald-100 text-emerald-700" },
];
...
const selectClass =
  "rounded-lg border border-neutral-300 bg-white px-3 py-2 text-sm text-neutral-800 outline-none transition-colors focus:border-teal-600 focus:ring-1 focus:ring-teal-600 hover:border-neutral-400";
...
function TicketRow({
  ...
  const statusColor =
    STATUS_COLORS[ticket.status] ?? "bg-neutral-100 text-neutral-700";
  const priorityColor =
    PRIORITY_COLORS[ticket.priority ?? ""] ?? "bg-neutral-100 text-neutral-500";
  ...
  return (
    ...
    >
      ...
      >
        {/* Index */}
        <span className="hidden w-8 flex-shrink-0 text-xs font-mono text-neutral-400 sm:block">
        ...
        <div className="border-t border-neutral-100 bg-neutral-50/50 px-4 py-4">
            ...
            <div className="rounded-lg border border-neutral-200 bg-white p-3">
              ...
              <ol className="space-y-0">
                {ticket.events
                  ...
                  .map((event, i, arr) => {
                    const dotColor =
                      EVENT_COLOR_MAP[event.type] ?? "bg-neutral-400";
                    const icon = EVENT_ICON_MAP[event.type] ?? (
                      <Activity className="h-3.5 w-3.5" />
                    );
                    const label =
                      EVENT_LABEL_MAP[event.type] ??
                    ...
                    return (
                      <li key={event.id} className="flex gap-3">
                        ...
                        <div className={`min-w-0 ${isLast ? "pb-0" : "pb-4"}`}>
                          ...
                          {event.note && (
                            <p className="mt-1 rounded-md bg-neutral-50 px-2 py-1 text-xs italic text-neutral-600">
          ...
          <div className="mt-3 flex flex-wrap items-center gap-2">
            <Link
              ...
              className="inline-flex items-center gap-1.5 rounded-lg bg-teal-600 px-3 py-1.5 text-xs font-semibold text-white shadow-sm transition-all hover:bg-teal-700 hover:shadow"
            >
              <Eye size={12} />
              Open full detail
            </Link>
            <button
              className="inline-flex items-center gap-1.5 rounded-lg border border-neutral-300 bg-white px-3 py-1.5 text-xs font-medium text-neutral-700 transition-colors hover:bg-neutral-50"
...
function MetaItem({
  ...
  return (
    <div className="rounded-lg border border-neutral-200 bg-white p-2">
      <p className="mb-0.5 flex items-center gap-1 text-[10px] font-bold uppercase tracking-widest text-neutral-400">
        {icon}
        {label}
      </p>
      <p
    ...
    <div className="px-4 py-6 sm:px-6 lg:px-8">
      ...
      <div className="mb-6 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            disabled={loading}
            className="inline-flex items-center gap-1.5 rounded-lg border border-neutral-300 bg-white px-3 py-2 text-sm font-medium text-neutral-700 shadow-sm transition-all hover:bg-neutral-50 disabled:opacity-50"
          >
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
          ...
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full rounded-lg border border-neutral-300 bg-white py-2 pl-9 pr-3 text-sm text-neutral-800 outline-none transition-colors focus:border-teal-600 focus:ring-1 focus:ring-teal-600"
            />
            {searchInput && (
              <button
      ...
      <div className="space-y-2">
        {!loading && !error && tickets.length === 0 && (
          <div className="rounded-xl border border-dashed border-neutral-300 bg-white p-12 text-center">
            <Inbox size={36} className="mx-auto mb-2 text-neutral-300" />
            ...
            {activeFilterCount > 0 && (
              <button
                onClick={clearFilters}
                className="mt-3 inline-flex items-center gap-1.5 rounded-lg bg-teal-600 px-3 py-1.5 text-xs font-semibold text-white hover:bg-teal-700"
              >
                <X size={12} />
                Clear filters
              </button>
      ...
      {totalPages > 1 && (
        <div className="mt-6 flex items-center justify-between border-t border-neutral-200 pt-4">
          <p className="text-xs text-neutral-500">
            Page <span className="font-semibold text-neutral-700">{page}</span>{" "}
            of{" "}
          ...
          <div className="flex items-center gap-1">
            <button
              ...
              className="inline-flex items-center gap-1 rounded-lg border border-neutral-300 bg-white px-3 py-1.5 text-xs font-medium text-neutral-700 shadow-sm transition-all hover:bg-neutral-50 disabled:cursor-not-allowed disabled:opacity-50"
            >
              <ChevronLeft size={12} />
              Prev
            </button>
