...
export default function IntakeDashboard() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const [overviewRes, trendRes, ticketsRes] = await Promise.all([
        ...
        api.get<PaginatedResponse<Ticket>>("/tickets?page=1&pageSize=5"),
      ]);
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-xl font-semibold text-neutral-800">Intake Dashboard</h1>
      ...
      <div className="grid grid-cols-2 gap-4 md:grid-cols-4">
        <StatCard
          ...
          accent="amber"
        />
        <StatCard label="Total Complaints" value={stats.total ?? 0} icon="total" accent="teal" />
            ...
            <div className="h-64">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={trend}>
                  ...
                  <Tooltip />
                  <Bar dataKey="total" fill="#0f766e" name="Complaints" />
                </BarChart>
              </ResponsiveContainer>
            </div>
          </div>
