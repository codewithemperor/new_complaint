...
/** Defines the ordered pipeline steps for the progress bar. */
...
function saveRecentSearch(code: string) {
  const existing = loadRecentSearches().filter((s) => s.code !== code);
  existing.unshift({ code, timestamp: Date.now() });
...
export default function TrackFormPage() {
  ...
  const lookup = useCallback(async () => {
    ...
    if (!code || !pin) {
      setError("Please enter both your ticket code and passcode.");
      return;
    }
    setLoading(true);
    setError(null);
    ...
    try {
      ...
      setError(
        err instanceof ApiError
          ? err.statusCode === 404
            ? "Invalid ticket code or passcode. Please check and try again."
            : err.message
  ...
  async function handleReply() {
    ...
    try {
      await api.post(
        `/tickets/${encodeURIComponent(code)}/info?passcode=${encodeURIComponent(pin)}`,
        { body: replyBody },
      );
      setReplyBody("");
      setReplySent(true);
  ...
  async function handleShare() {
    const url = `${window.location.origin}/track?code=${encodeURIComponent(ticketCode.trim().toUpperCase())}&passcode=${encodeURIComponent(passcode.trim())}`;
    try {
  ...
  function handleRecentSearchClick(code: string) {
    setTicketCode(code);
    setPasscode("");
    setError(null);
  }
  ...
  const progressInfo = useMemo(() => {
    ...
    return {
      ...
      percent: Math.round(((idx + 1) / PIPELINE_STEPS.length) * 100),
    };
  }, [ticket]);
              ...
              <div className="space-y-4 p-6">
                ...
                <div>
                  ...
                  <p className="mt-1 text-xs text-neutral-400">
                    Found in your complaint confirmation email or SMS
                  </p>
                </div>
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <ul className="mt-2 space-y-1">
                              <li className="flex items-center gap-1.5 text-xs text-red-600">
                                ...
                                Double-check your ticket code for typos
                              </li>
                              <li className="flex items-center gap-1.5 text-xs text-red-600">
                                <span className="h-1 w-1 rounded-full bg-red-400" />
                                Verify the passcode matches your confirmation
                              </li>
                              <li className="flex items-center gap-1.5 text-xs text-red-600">
                                <span className="h-1 w-1 rounded-full bg-red-400" />
              ...
              <AnimatePresence>
                {recentSearches.length > 0 && !error && (
                  <motion.div
                    initial={{ opacity: 0 }}
                    animate={{ opacity: 1 }}
                    className="border-t border-neutral-100 bg-neutral-50/50 px-6 py-4"
                  >
                  ...
                  <div key={step} className="flex flex-col items-center">
                    <div className="flex items-center gap-0.5">
                      ...
                      {i < PIPELINE_STEPS.length - 1 && (
                        <div className="h-0.5 w-3 bg-neutral-200" />
                      )}
                    </div>
                    <p className="mt-1 text-[8px] font-medium text-neutral-500 text-center leading-tight max-w-[50px]">
                      {STATUS_LABELS[step]}
                    </p>
                  </div>
                ))}
  ...
  return (
    <div className="min-h-screen bg-neutral-50">
      ...
      <div className="mx-auto max-w-lg px-4 pb-8 pt-24">
        ...
        {/* Ticket detail card with glass morphism */}
        <motion.div
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
          ...
          <div className="space-y-5 p-6">
            ...
            {ticket.timeline && ticket.timeline.length > 0 && (
              <div>
                ...
                <div className="relative">
                  {/* Fallback single line for simplicity */}
            ...
            <div className="flex gap-2">
              ...
              <button
                onClick={() => {
                  // Placeholder: generate a simple text report
