...
let ReportsService = class ReportsService {
    ...
    /** Per-department performance metrics within an optional date range. */ async departmentPerformance(from, to) {
        ...
        return departments.map((d)=>{
            ...
            const resolutionHours = resolved.filter((t)=>t.resolvedAt && t.slaStartedAt).map((t)=>(t.resolvedAt.getTime() - t.slaStartedAt.getTime()) / 3_600_000);
            const avgResolutionHours = resolutionHours.length ? Math.round(resolutionHours.reduce((a, b)=>a + b, 0) / resolutionHours.length * 10) / 10 : null;
            return {
                departmentId: d.id,
    ...
    /** Per-officer performance metrics within an optional date range. */ async officerPerformance(from, to) {
        ...
        return officers.filter((o)=>o.assignedTickets.length > 0).map((o)=>{
    ...
    /** Daily ticket count over the last N days (default 30) for charts. */ async trend(days = 30) {
        ...
        return Object.entries(grouped).sort(([a], [b])=>a.localeCompare(b)).map(([date, counts])=>({
                date,
                ...counts
            }));
    }
